...
async def current_user_get_info_controller(
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(f"{settings.INFO_SERVICE_HOST}/api/info", headers=headers)
            response.raise_for_status()  # Вызывает исключение, если статус ответа >= 400
            data = response.json()
            return InfoResponse(**data)
        except httpx.HTTPStatusError as e:
            logger.error(f"Info Service returned an error: {e.response.status_code} - {e.response.text}")
            raise HTTPException(status_code=e.response.status_code, detail="Failed to fetch info")
