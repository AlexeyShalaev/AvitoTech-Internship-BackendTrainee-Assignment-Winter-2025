...
def convert_sql_models_to_pydantic_schemas(models: iter, schema: BaseModel) -> list[BaseModel]:
    return [convert_sql_model_to_pydantic_schema(row, schema) for row in models]


def convert_first_sql_model_in_array_to_pydantic_schema(models: list[Any] | None, schema: BaseModel) -> BaseModel:
