...
def upgrade() -> None:
    ...
    sa.CheckConstraint('balance_whole >= 0 AND balance_fraction >= 0 AND balance_fraction < 100', name=op.f('accounts_check_balance_valid_check')),
    ...
    sa.Column('status', sa.Enum('COMPLETED', 'ROLLBACKED', name='transaction_status'), nullable=False),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text("timezone('UTC', now())"), nullable=False),
    sa.CheckConstraint('amount_fraction >= 0 AND amount_fraction < 100', name=op.f('transactions_check_transaction_fraction_check')),
    sa.ForeignKeyConstraint(['from_account_id'], ['accounts.id'], name=op.f('transactions_from_account_id_fkey')),
    sa.ForeignKeyConstraint(['to_account_id'], ['accounts.id'], name=op.f('transactions_to_account_id_fkey')),
