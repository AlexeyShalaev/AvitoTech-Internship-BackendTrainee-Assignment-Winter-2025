...
class CoinsService:
    ...
    async def _publish_transaction(self, transaction: Transaction):
        message = {
            ...
            "from_account_id": (
                str(transaction.from_account_id)
            ...
            "from_username": (
                str(transaction.from_username) if transaction.from_username else None
            ),
            "to_username": (
                str(transaction.to_username) if transaction.to_username else None
            ),
        ...
        try:
            ...
            await KafkaProducerSingleton.get_producer().send_and_wait(
                settings.KAFKA_TX_TOPIC_NAME, value=orjson.dumps(message)
            )
            logger.info(f"Transaction {transaction.id} published to Kafka")
        except Exception as e:
            logger.error(
                f"Failed to publish transaction {transaction.id} to Kafka: {e}"
            )

    async def TransferFunds(
        self, request: coins_pb2.TransferFundsRequest
    ) -> coins_pb2.TransferFundsResponse:
        logger.info(
            f"Starting TransferFunds from {request.from_username} to {request.to_username}, amount: {request.amount_whole}.{request.amount_fraction}"
        )

        if await self._check_idempotency(request.idempotency_key):
            logger.warning(
                f"Duplicate transfer request with idempotency key {request.idempotency_key}"
            )
            raise GrpcException(
        ...
        if (
            request.amount_whole < 0
            or request.amount_fraction < 0
            or request.amount_fraction >= 100
            or request.amount_whole == 0
            and request.amount_fraction == 0
        ):
            logger.warning(
                f"Invalid amount: {request.amount_whole}.{request.amount_fraction}"
            )
            raise GrpcException(
        ...
        async with self._db_session.begin():
            ...
            stmt = (
                select(Account).where(Account.username.in_(usernames)).with_for_update()
            )
            result = await self._db_session.execute(stmt)
            accounts = {acc.username: acc for acc in result.scalars()}

            if (
                request.from_username not in accounts
                or request.to_username not in accounts
            ):
            ...
            from_acc, to_acc = (
                accounts[request.from_username],
                accounts[request.to_username],
            )
            ...
            if total_whole < 0:
                logger.error(
                    f"Insufficient funds for {request.from_username}: requested {request.amount_whole}.{request.amount_fraction}, available {from_acc.balance_whole}.{from_acc.balance_fraction}"
                )
                raise GrpcException(
                    status_code=grpc.StatusCode.FAILED_PRECONDITION,
        ...
        return coins_pb2.TransferFundsResponse(
            transaction_id=str(transaction.id), status=coins_pb2.Status.COMPLETED
        )

    async def ChargeUser(
        self, request: coins_pb2.ChargeUserRequest
    ) -> coins_pb2.ChargeUserResponse:
        logger.info(
            f"Starting ChargeUser for {request.username}, amount: {request.amount_whole}.{request.amount_fraction}"
        )

        if await self._check_idempotency(request.idempotency_key):
            logger.warning(
                f"Duplicate charge request with idempotency key {request.idempotency_key}"
            )
            raise GrpcException(
                status_code=grpc.StatusCode.ALREADY_EXISTS,
                details=self.ProblemCode.DUPLICATE_REQUEST,
        ...
        async with self._db_session.begin():
            ...
            stmt = (
                select(Account)
                .where(Account.username == request.username)
                .with_for_update()
            )
            result = await self._db_session.execute(stmt)
            ...
            if total_whole < 0:
                logger.error(
                    f"Insufficient funds for {request.username}: requested {request.amount_whole}.{request.amount_fraction}, available {account.balance_whole}.{account.balance_fraction}"
                )
                raise GrpcException(
                    status_code=grpc.StatusCode.FAILED_PRECONDITION,
                    details=self.ProblemCode.INSUFFICIENT_FUNDS,
        ...
        await self._db_session.commit()
        logger.info(
            f"Charge complete for {request.username}, transaction_id={transaction.id}"
        )
    ...
    async def CreditUser(
        self, request: coins_pb2.CreditUserRequest
    ) -> coins_pb2.CreditUserResponse:
        logger.info(
            f"Starting CreditUser for {request.username}, amount: {request.amount_whole}.{request.amount_fraction}"
        )

        if await self._check_idempotency(request.idempotency_key):
            logger.warning(
                f"Duplicate credit request with idempotency key {request.idempotency_key}"
            )
            raise GrpcException(
                status_code=grpc.StatusCode.ALREADY_EXISTS,
                details=self.ProblemCode.DUPLICATE_REQUEST,
        ...
        await self._db_session.commit()
        logger.info(
            f"Credit complete for {request.username}, transaction_id={transaction.id}"
        )

        await self._publish_transaction(transaction)

        return coins_pb2.CreditUserResponse(
            transaction_id=str(transaction.id), status=coins_pb2.Status.COMPLETED
        )

    async def GetBalance(
        self, request: coins_pb2.GetBalanceRequest
        ...
        logger.info(
            f"User {request.username} balance: {account.balance_whole}.{account.balance_fraction}"
        )
    ...
    async def GetTransactionHistory(
        self, request: coins_pb2.GetTransactionHistoryRequest
