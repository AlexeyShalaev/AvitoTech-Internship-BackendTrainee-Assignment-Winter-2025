    ...
    async def _publish_selling(
        ...
        try:
            await KafkaProducerSingleton.get_producer().send_and_wait(
                settings.KAFKA_MERCH_TOPIC_NAME, value=orjson.dumps(message)
            )
        except Exception as e:
            logger.error(f"Failed to publish selling to Kafka: {e}")

    async def BuyMerch(
        self, request: merch_pb2.BuyMerchRequest
    ) -> merch_pb2.BuyMerchResponse:
        logger.info(
            f"BuyMerch request: {request.username}, {request.merch_name}, {request.idempotency_key}"
        )
        ...
        if not merch:
            raise GrpcException(
                grpc.StatusCode.NOT_FOUND, self.ProblemCode.MERCH_NOT_FOUND
            )

        try:
            async with CoinsServiceClient() as coins_service:
                response: coins_pb2.ChargeUserResponse = (
                    await coins_service.charge_user(
        ...
        await self._publish_selling(
            response.transaction_id, status, request.username, merch.name, merch.price
        )

        return merch_pb2.BuyMerchResponse(
            transaction_id=response.transaction_id, status=status
        )
